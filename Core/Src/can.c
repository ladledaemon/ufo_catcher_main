...
{
  CANHandle_StdID* _can_handle = (CANHandle_StdID*)malloc(sizeof(CANHandle_StdID));
  if(RegisterCANHandle(&_can_handle) != HAL_OK){
    Error_Handler();
  }
  _can_handle->phcan = _hcan;
  ...
  if(_hcan->Instance == CAN1){
    FilterId = MakeStdId(CURRENT_STATE, 0, 0) << 21;
    ...
    FilterId = MakeStdId(FEEDBACK, 0, 0) << 21;
